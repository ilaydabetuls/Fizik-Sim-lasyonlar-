...
const postTest1Questions = [
  {
    ...
    options: [
      ...
      { id: 'C', text: 'Zamanla azalır' },
      { id: 'D', text: 'Değişkene bağlıdır' }
    ]
  }
];
...
export default function PostTest() {
  ...
  useEffect(() => {
    const qId = postTest1Questions[currentIndex].id;
  ...
  if (isCompleted) return <div className="p-20 text-center"><div className="text-green-500 text-5xl">✔</div><h2 className="text-xl font-bold">Son testiniz başarıyla tamamlandı.</h2><p>Araştırmaya katıldığınız için teşekkürler!</p><button onClick={() => navigate('/')} className="mt-4 bg-blue-600 text-white px-4 py-2 rounded">Çıkış</button></div>;

  const q = postTest1Questions[currentIndex];
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <header className="bg-white shadow p-4 flex justify-between items-center max-w-4xl mx-auto w-full"><span className="font-bold">Soru {currentIndex + 1}/{postTest1Questions.length}</span><div className="flex space-x-2">{postTest1Questions.map((_, i) => <button key={i} onClick={() => setCurrentIndex(i)} className={`w-8 h-8 rounded-full border ${i === currentIndex ? 'border-blue-600 bg-blue-50' : answers[postTest1Questions[i].id] ? 'bg-blue-600 text-white' : 'bg-white'}`}>{i + 1}</button>)}</div></header>
      ...
      <footer className="bg-white border-t p-4 flex justify-between max-w-4xl mx-auto w-full"><button onClick={() => setCurrentIndex(p => p - 1)} disabled={currentIndex === 0} className="px-6 py-2 bg-gray-200 rounded disabled:opacity-50">Önceki</button>{currentIndex === postTest1Questions.length - 1 ? <button onClick={submit} className="px-6 py-2 bg-green-600 text-white rounded font-bold">Bitir</button> : <button onClick={() => setCurrentIndex(p => p + 1)} className="px-6 py-2 bg-blue-600 text-white rounded">Sonraki</button>}</footer>
    </div>
  );
}
